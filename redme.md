...
| (Attacker)          |   --->  | (Target)             |   --->  | (Log Monitoring)   |
| IP: 172.16.0.10     |         | IP: 172.16.0.20      |   --->  | (on Host)          |
+---------------------+         +----------------------+         | IP: 172.16.0.1     |
          |                            |                              +--------------------+
          |                            |                              ^
          |                            |                              |
          +----------------------------+------------------------------+
                  (Host-Only Network Adapter - VMnet1)

The Kali Linux machine initiates attacks against the Windows 10 VM. Logs generated by Sysmon on the Windows machine are collected by Splunk running on the Host machine via the Host-Only network for analysis and detection of malicious activities.